...
def get_metrics(data, labels):
    """Calculates technical clustering performance scores."""
    if len(set(labels)) < 2: 
...
def get_cluster_profiles(df, labels):
    """Returns the average values for each cluster."""
    df_temp = df.copy()
...
def get_recommendations(profile):
    """
    Analyzes column names and values to generate specific business advice.
    Works for ANY dataset by checking keywords in column headers.
    """
    recs = {}
    ...
    for cluster_id, row in profile.iterrows():
        ...
        # Compare this cluster's averages to the global average
        # We look for features that are significantly different (+/-)
        impacts = []
        ...
        for col in row.index:
            ...
            if global_val == 0: global_val = 1e-9 # Avoid zero division
            
            # Calculate % difference
            diff = (cluster_val - global_val) / abs(global_val)
            impacts.append((col, diff))
            
        # Sort by biggest impact (absolute difference)
        impacts.sort(key=lambda x: abs(x[1]), reverse=True)
        top_features = impacts[:3] # Focus on top 3 distinguishing traits
        ...
        for feature, diff in top_features:
            ...
            # 2. Frequency / Engagement / Activity
            ...
            # 4. Fallback for unknown columns (Health, Grades, etc.)
            else:
                if direction == "High":
...
def generate_segment_names(profile):
    ...
    for cid, row in profile.iterrows():
        # Find the single most defining feature
        max_diff = 0
        best_tag = "Standard"
        ...
        for col in row.index:
            ...
            # If difference is significant (>25%) and bigger than previous best
